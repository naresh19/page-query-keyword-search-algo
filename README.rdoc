I have written 3 classes Page , Query, and Search each in file page.rb , query.rb, search.rb and order.rb  . It reads pages and queries from sample_data.txt in the same directory as the file containing code to get te neccessary data to search top pages for each query.

Tu run from console :
ruby search.rb (which will call Order.excute)
It will take 2 inputs
	1. Path of sample file
	2. Total Number of words
	(by default it take sample_data.txt and 8)
It will output all queries result.

Or to directly run test cases
comment this line
Search.execute
...
Solve the problem described below using​a Programming Language ​
of your choice. The objective of the problem is to allow you to demonstrate two aspects:­
